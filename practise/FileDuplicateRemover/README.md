...
> 小黑沿超长文件只挖几个采样点算哈希，而非通读整条文件；最后还偷偷把"文件大小"牌子丢进哈希桶，防止内容被伪造骗过去。
...
> 上图描绘了工具的整体运行链路：从左往右，小黑先扫描目录把文件路径抛进通道，再分发给多个工人并发计算 SHA-256 哈希，最后汇聚收集、挑出重复组、交互删除并丢进回收站。橙色管道代表 channel 数据流，红色圈出"重复"文件。
...
> 启动后，小黑先当"安检员"：校验 `-p` 指向的路径是否真实存在且为目录、`-t` 类型是否落在 `music / video / image` 白名单内。任一不合法都会被直接拦下并退出，不会进入扫描阶段。
...
> 上：同盘直接一推即达；下：跨盘时旧文件、新盒子两个句柄必须在删除前先关好，否则 Windows 不让删。
...
> 小黑拉着贯穿整条流水线的总绳，一拽全停；遇到带锁的目录耸肩跳过，遇到 panic 红牌立即刹停。
